...
<h1>lpac 2.3.0 QMI/AT only Backport for OpenWrt & eSIM Management Toolchain</h1>
...
<h2>Что проделано</h2>
<ul>
    <li><b>Бэкпорт lpac 2.3.0:</b> Версия адаптирована для работы под musl libc в среде OpenWrt 21.x – 24.x.</li>
    ...
    <li><b>lpac-esim-qmi.sh:</b> Разработан интерактивный BusyBox-совместимый скрипт с логикой «умного» сброса модема.</li>
  <h2>Ключевые архитектурные изменения:</h2>
 <ul>
 <li><b> Монолитная сборка (Static Backends): Выполнен отказ от внешней системы драйверов (.so). <li>Поддержка протоколов QMI, AT и HTTP (CURL) теперь жестко вкомпилирована в основной бинарный файл lpac. 
<li><b>Удаление избыточных зависимостей: Из сборочного контура и runtime-зависимостей исключены mbim и qrtr, пакет максимально облегчен под целевой сценарий.</li>
<ul>
<li><b>ВНИМАНИЕ: MBIM пока что в билде отсутствует!</li>
</ul>
